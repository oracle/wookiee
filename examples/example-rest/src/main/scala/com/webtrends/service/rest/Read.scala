...
import com.webtrends.harness.component.spray.route.{SprayHead, SprayOptions, SprayGet}
...
class Read extends Command
   ...
   * @return
   */
  override def commandName: String = Read.CommandName
  ...
  def execute[T](bean: Option[CommandBean]): Future[CommandResponse[T]] = {
    ...
    bean match {
      ...
      case None => p failure new CommandException("Read", "Cache not initialized")
    }
    p.future
  }
}
