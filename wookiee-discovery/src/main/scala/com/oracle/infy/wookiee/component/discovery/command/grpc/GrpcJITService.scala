        ...
        asyncUnaryCall(new ServerCalls.UnaryMethod[StringValue, StringValue]() {
          ...
          override def invoke(request: StringValue, responseObserver: StreamObserver[StringValue]): Unit =
            try {
              ...
              log.debug(s"Invoked [${command.commandName}] with request [${request.getValue}]")
              val input = command.jsonToInput(request.getValue)
              // Invoke the command
              command.execute(input) onComplete {
                case Success(output) =>
                  log.debug(s"Successfully invoked [${command.commandName}] with request [${request.getValue}]")
                ...
                case Failure(ex) =>
                  ...
                  log.error(s"Error invoking [${command.commandName}] with request [${request.getValue}]", ex)
                  responseObserver.onError(ex)
              }
            } catch {
              case ex: Throwable =>
                // Issue happened on the client side
                log.error(s"Error in initial get of [${command.commandName}] with request [${request.getValue}]", ex)
                responseObserver.onError(ex)
            }
        })
      )
