...
private[harness] trait LogProcessor extends BaseLogProcessor {

  def process(event: LogEvent) = event match {
    case e@(le: LogEvent) if e.marker.isDefined => processMarkerEvent(e)
    case e => processLoggerEvent(e)
  }

  private def processLoggerEvent(event: LogEvent) = event match {
    case Error(logger, message, _, altSource, params, cause) =>
      cause match {
    ...
    case Warn(logger, message, _, altSource, params, cause) =>
      cause match {
        case Some(t) => withContext(event.thread, event.timestamp, event.altSource) {
    ...
    case Info(logger, message, _, altSource, params, cause) =>
      cause match {
        case Some(t) => withContext(event.thread, event.timestamp, event.altSource) {
    ...
    case Debug(logger, message, _, altSource, params, cause) =>
      cause match {
        case Some(t) => withContext(event.thread, event.timestamp, event.altSource) {
    ...
    case Trace(logger, message, _, altSource, params, cause) =>
      cause match {
        case Some(t) => withContext(event.thread, event.timestamp, event.altSource) {
