...
class ZookeeperSettingsSpec extends SpecificationWithJUnit {
  ...
  "ZookeeperSettings" should {
    "load properly with valid configuration" in {
      ...
      val subConfig = ConfigUtil.prepareSubConfig(config, "wookie-zookeeper")

      val settings = ZookeeperSettings(subConfig)
      settings.quorum must be equalTo config.getString("wookie-zookeeper.quorum")
      30000L must be equalTo config.getDuration("wookie-zookeeper.session-timeout", TimeUnit.MILLISECONDS)
      5000L must be equalTo config.getDuration("wookie-zookeeper.retry-sleep", TimeUnit.MILLISECONDS)
    }
    ...
    ConfigFactory.parseString(
      """
        wookie-zookeeper {
          ...
          libsLocation = "../components/wookie-zookeeper/target/lib"
          jarLocation = "../components/wookie-zookeeper/target/wookie-zookeeper-1.0-SNAPSHOT.jar"
