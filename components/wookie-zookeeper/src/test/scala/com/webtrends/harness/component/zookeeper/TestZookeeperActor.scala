...
object TestZookeeperActor {
  ...
  private def getConfig(zookeeperQuorum: String)(implicit system: ActorSystem): ZookeeperSettings = {
    if (!zookeeperQuorum.isEmpty) {
      val conf = "wookie-zookeeper.quorum=\"" + zookeeperQuorum + "\""
      ZookeeperSettings(TestConfig.conf(conf))
    }
    if (system.settings.config.hasPath("wookie-zookeeper")) {
      ZookeeperSettings(system.settings.config.getConfig("wookie-zookeeper"))
    } else {
...
class TestZookeeperActor(settings: ZookeeperSettings) extends ZookeeperActor(settings) {
  log.info(s"Create the TestZookeeperActor and attaching to Zookeeper at ${context.system.settings.config.getString("wookie-zookeeper.quorum")}")
}
