...
object NodeRegistration {
  ...
  def getBasePath(config: Config): String = {
    val zookeeperSettings = ZookeeperSettings(config.getConfig("wookie-zookeeper"))
    val basePath = if (config.hasPath("wookie-cluster.base-path")) {
      config.getConfig("wookie-cluster").getString("base-path")
    } else {
