...
protected class CoreHttpHandler(settings: NettyServerSettings, worker: ActorRef) extends BaseInboundHandler {
  ...
  override def handleHttpRequest(ctx: ChannelHandlerContext, req: FullHttpRequest): Boolean = {
    ...
    (req.getMethod, req.getUri) match {
      ...
      case (GET, "/metrics") =>
        (worker ? GetSystemInfo("wookie-metrics", ComponentRequest(StatusRequest()))).mapTo[ComponentResponse[JValue]] onComplete {
      ...
      case (GET, "/cluster") =>
        (worker ? GetSystemInfo("wookie-cluster", ComponentRequest(ClusterState(), Some("cluster")))).mapTo[ComponentResponse[JValue]] onComplete {
          case Success(s) => sendContent(ctx, req, JsonAST.compactRender(s.resp), "application/json")
          case Failure(f) => sendFailure(ctx, req, f)
        }
      case (GET, "/cluster/discovery") =>
        (worker ? GetSystemInfo("wookie-cluster", ComponentRequest(Subscriptions()))).mapTo[ComponentResponse[JValue]] onComplete {
          case Success(s) => sendContent(ctx, req, JsonAST.compactRender(s.resp), "application/json")
          case Failure(f) => sendFailure(ctx, req, f)
        }
