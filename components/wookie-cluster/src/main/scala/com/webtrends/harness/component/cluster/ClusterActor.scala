...
class ClusterActor extends Actor
  ...
  val zookeeperSettings = ZookeeperSettings(context.system.settings.config.getConfig("wookie-zookeeper"))
  val clusterSettings = ClusterSettings(context.system.settings.config.getConfig("wookie-cluster"), akkaProvider)
