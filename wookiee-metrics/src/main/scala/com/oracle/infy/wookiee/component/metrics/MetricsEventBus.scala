...
import com.oracle.infy.wookiee.component.metrics.messages.MetricMessage
...
object MetricsEventBus {
  ...
  /**
    ...
    */
  private[oracle] def subscribe(subscriber: ActorRef): Unit = {
    bus.subscribe(subscriber, true)
    ()
  }
  ...
  /**
    ...
    */
  private[oracle] def unsubscribe(subscriber: ActorRef): Unit = bus.unsubscribe(subscriber)

  /**
    * Publish the metric message
    * @param message the metric message to publish
    */
  private[metrics] def publish(message: MetricMessage): Unit = bus.publish(message)
