...
import com.webtrends.harness.libs.iteratee.internal.{ eagerFuture, executeFuture }
import scala.concurrent.{ ExecutionContext, Future, Promise, blocking }
import scala.util.{ Try, Success, Failure }
import scala.language.reflectiveCalls

/**
 * A producer which pushes input to an [[com.webtrends.harness.libs.iteratee.Iteratee]].
 *
...
trait Enumerator[E] {
  ...
  /**
   * Attaches this Enumerator to an [[com.webtrends.harness.libs.iteratee.Iteratee]], driving the
   * Iteratee to (asynchronously) consume the input. The Iteratee may enter its
   * [[com.webtrends.harness.libs.iteratee.Done]] or [[com.webtrends.harness.libs.iteratee.Error]]
   * state, or it may be left in a [[com.webtrends.harness.libs.iteratee.Cont]] state (allowing it
   * to consume more input after that sent by the enumerator).
   *
   * If the Iteratee reaches a [[com.webtrends.harness.libs.iteratee.Done]] state, it will
   * contain a computed result and the remaining (unconsumed) input.
   */
  def apply[A](i: Iteratee[E, A]): Future[Iteratee[E, A]]

  /**
   * Alias for `apply`, produces input driving the given [[com.webtrends.harness.libs.iteratee.Iteratee]]
   * to consume it.
   */
   ...
   * reach a [[com.webtrends.harness.libs.iteratee.Done]] or [[com.webtrends.harness.libs.iteratee.Error]]
   * state when the Enumerator finishes, this method forces one of those states by
   * feeding `Input.EOF` to the Iteratee.
   *
   * If the iteratee is left in a [[com.webtrends.harness.libs.iteratee.Done]]
   * state then the promise is completed with the iteratee's result.
   * If the iteratee is left in an [[com.webtrends.harness.libs.iteratee.Error]] state, then the
...
object Enumerator {
  ...
  /**
   * Like [[com.webtrends.harness.libs.iteratee.Enumerator.unfold]], but allows the unfolding to be done asynchronously.
   *
  ...
  /**
   * Like [[com.webtrends.harness.libs.iteratee.Enumerator.repeat]], but allows repeated values to be asynchronously fetched.
   *
   * @param e The input function
   * $paramEcSingle
   */
  ...
  /**
   * Like [[com.webtrends.harness.libs.iteratee.Enumerator.repeatM]], but the callback returns an Option, which allows the stream
  ...
  /**
   * Like [[com.webtrends.harness.libs.iteratee.Enumerator.generateM]], but `retriever` accepts a boolean
