...
import com.webtrends.harness.libs.iteratee.internal.{eagerFuture, executeFuture, executeIteratee, prepared}
...
object Iteratee {
  ...
  /**
   * flatten a [[scala.concurrent.Future]] of [[com.webtrends.harness.libs.iteratee.Iteratee]]] into an Iteratee
   *
  ...
  /**
   * Create an [[com.webtrends.harness.libs.iteratee.Iteratee]] which folds the content of the Input using a given function and an initial state
   *
   * Example:
   * {{{
  ...
  def foldM[E, A](state: A)(f: (A, E) => Future[A])(implicit ec: ExecutionContext): Iteratee[E, A] = {
    def step(s: A)(i: Input[E]): Iteratee[E, A] = i match {
      ...
      case Input.El(e) =>
        val newS = executeFuture(f(s, e))(ec)
  ...
  trait EofOrElse[E] {
    /**
     * @param otherwise Value if the input is not [[com.webtrends.harness.libs.iteratee.Input.EOF]]
     * @param eofValue Value if the input is [[com.webtrends.harness.libs.iteratee.Input.EOF]]
     * @tparam A Type of `eofValue`
     * @tparam B Type of `otherwise`
     * @return An `Iteratee[E, Either[B, A]]` that consumes one input and produces a `Right(eofValue)` if this input is [[com.webtrends.harness.libs.iteratee.Input.EOF]] otherwise it produces a `Left(otherwise)`
     */
    def apply[A, B](otherwise: => B)(eofValue: A): Iteratee[E, Either[B, A]]
  }

  def eofOrElse[E]: EofOrElse[E] = new EofOrElse[E] {
    def apply[A, B](otherwise: => B)(eofValue: A): Iteratee[E, Either[B, A]] = {
      def cont: Iteratee[E, Either[B, A]] = Cont {
        case in@Input.El(_) => Done(Left(otherwise), in)
        case in@Input.EOF => Done(Right(eofValue), in)
  ...
  /**
   * @return an [[com.webtrends.harness.libs.iteratee.Iteratee]] which just ignores its input
   */
  def ignore[E]: Iteratee[E, Unit] = fold[E, Unit](())((_, _) => ())(dec)

  /**
   * @return an [[com.webtrends.harness.libs.iteratee.Iteratee]] which executes a provided function for every chunk. Returns Done on EOF.
   *
   * Example:
   * {{{
  ...
  /**
   *
   * @return an [[com.webtrends.harness.libs.iteratee.Iteratee]] which pushes the input into the provided [[com.webtrends.harness.libs.iteratee.Iteratee]], starting over again each time it terminates until an EOF is received, collecting a sequence of results of the different use of the iteratee
   *
  ...
  def repeat[E, A](i: Iteratee[E, A]): Iteratee[E, Seq[A]] = {
    ...
    def step(s: Seq[A])(input: Input[E]): Iteratee[E, Seq[A]] = {
      input match {
        ...
        case Input.El(_) => i.pureFlatFold {
          case Step.Done(a, _) => Done(s :+ a, input)
...
sealed trait Step[E, +A] {
  ...
  // This version is not called by Step implementations in wookiee,
 ...
 * (see the [[com.webtrends.harness.libs.iteratee.Step]] trait):
 * [[com.webtrends.harness.libs.iteratee.Done]], which means it contains a result and potentially some unconsumed part of the stream;
 * [[com.webtrends.harness.libs.iteratee.Cont]], which means it contains a function to be invoked to generate a new Iteratee from the next piece of input;
 * [[com.webtrends.harness.libs.iteratee.Error]], which means it contains an error message and potentially some unconsumed part of the stream.
 *
 ...
 *
 * Typically an [[com.webtrends.harness.libs.iteratee.Enumerator]] would be used to
 * push data into an Iteratee by invoking the function in the [[com.webtrends.harness.libs.iteratee.Cont]]
 ...
 * The state of an Iteratee (the current [[com.webtrends.harness.libs.iteratee.Step]] may not be available
...
trait Iteratee[E, +A] {
   ...
   * if the Iteratee is in the [[com.webtrends.harness.libs.iteratee.Cont]] state.
  ...
  def run: Future[A] = fold({
    ...
    case Step.Cont(k) => k(Input.EOF).fold({
      ...
      case Step.Error(msg, _) => sys.error(msg)
    })(dec)
    case Step.Error(msg, _) => sys.error(msg)
  })(dec)
   ...
   */
  protected[harness] def foldNoEC[B](folder: Step[E, A] => Future[B]): Future[B] =
    fold(folder)(dec)
   ...
   * which can execute the `folder` function immediately.
   */
  protected[harness] def pureFoldNoEC[B](folder: Step[E, A] => B): Future[B] =
    pureFold(folder)(dec)
   ...
   * which can execute the `folder` function immediately.
   */
  protected[harness] def pureFlatFoldNoEC[B, C](folder: Step[E, A] => Iteratee[B, C]): Iteratee[B, C] =
  ...
  def flatMap[B](f: A => Iteratee[E, B])(implicit ec: ExecutionContext): Iteratee[E, B] = {
    self.pureFlatFoldNoEC { // safe: folder either yields value immediately or executes with another EC
      ...
      case Step.Cont(k) =>
        Cont((in: Input[E]) => executeIteratee(k(in))(dec).flatMap(f)(ec))
      case Step.Error(msg, e) => Error(msg, e)
    }
  }
  ...
  def recoverM[B >: A](pf: PartialFunction[Throwable, Future[B]])(implicit ec: ExecutionContext): Iteratee[E, B] = {
    recoverWith { case t: Throwable if pf.isDefinedAt(t) => Iteratee.flatten(pf(t).map(b => Done[E, B](b))(ec)) }(ec)
  }
  ...
  def recoverWith[B >: A](pf: PartialFunction[Throwable, Iteratee[E, B]])(implicit ec: ExecutionContext): Iteratee[E, B] = {
    def recoveringIteratee(it: Iteratee[E, A]): Iteratee[E, B] = {
      val futureRecoveringIteratee: Future[Iteratee[E, B]] = it.pureFlatFold[E, B] {
        ...
        case done => done.it
      }(dec).unflatten.map(_.it)(dec).recover(pf)(ec)
    ...
    this.flatMap { a =>
      ...
      inner.pureFlatFold[E, AIn] {
        ...
        case Step.Cont(k) => k(Input.EOF).pureFlatFold[E, AIn] {
          case Step.Done(a, _) => Done(a, Input.Empty)
          case Step.Cont(_) => Error("divergent inner iteratee on joinI after EOF", Input.EOF)
          case Step.Error(msg, _) => Error(msg, Input.EOF)
        }(dec)
        case Step.Error(msg, _) => Error(msg, Input.Empty)
      }(dec)
    }(dec)
  }
}
...
private sealed trait StepIteratee[E, A] extends Iteratee[E, A] with Step[E, A] {
  ...
  protected[harness] final override def foldNoEC[B](folder: Step[E, A] => Future[B]): Future[B] = {
    folder(immediateUnflatten)
  }

  protected[harness] final override def pureFoldNoEC[B](folder: Step[E, A] => B): Future[B] = {
    try Future.successful(folder(immediateUnflatten)) catch { case NonFatal(e) => Future.failed(e) }
  }

  protected[harness] final override def pureFlatFoldNoEC[B, C](folder: Step[E, A] => Iteratee[B, C]): Iteratee[B, C] = {
    folder(immediateUnflatten)
  }
...
private final class DoneIteratee[E, A](a: A, e: Input[E]) extends Step.Done[A, E](a, e) with StepIteratee[E, A] {
  ...
  /**
   * Use an optimized implementation because this method is called by wookiee when running an
...
private final class FutureIteratee[E, A](itFut: Future[Iteratee[E, A]]) extends Iteratee[E, A] {
  ...
  def fold[B](folder: Step[E, A] => Future[B])(implicit ec: ExecutionContext): Future[B] = {
    itFut.flatMap { it => it.fold(folder)(ec) }(dec)
  }
...
object Done {
  /**
   * Create an [[com.webtrends.harness.libs.iteratee.Iteratee]] in the “done” state.
   * @param a Result
...
object Cont {
  /**
   * Create an [[com.webtrends.harness.libs.iteratee.Iteratee]] in the “cont” state.
...
object Error {
  /**
   * Create an [[com.webtrends.harness.libs.iteratee.Iteratee]] in the “error” state.
