...
import com.webtrends.harness.component.ComponentReloadActor.CheckForChanges
...
class ComponentReloadActor(loader: HarnessClassLoader) extends HActor with ComponentHelper {
  ...
  def changeChecker(): Future[Unit] = Future {
    try {
      ...
      val events = key.pollEvents().asScala.toStream.takeWhile(_.kind() != OVERFLOW).toList
      events.foreach {
        event =>
