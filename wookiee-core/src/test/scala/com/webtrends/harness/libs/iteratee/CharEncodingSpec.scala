...
class CharEncodingSpec extends WordSpecLike with MustMatchers {
  ...
  "CharEncodingSpec.decode()" should {
    ...
    "decode US-ASCII" in {
      ...
      Await.result(result, Duration.Inf) mustBe new String(input.flatten.toArray, "US-ASCII")
    }
    ...
    "decode UTF-8" in {
      ...
      Await.result(result, Duration.Inf) mustBe new String(input.flatten.toArray, "UTF-8")
    }
    ...
    "decode UTF-8 with split characters" in {
      ...
      val result = Enumerator(input: _*) &> CharEncoding.decode("UTF-8") |>>> Iteratee.consume[String]()
      Await.result(result, Duration.Inf) mustBe "\u20ac\ud83c\udca5"
    }
    ...
    "decode UTF-16" in {
      ...
      Await.result(result, Duration.Inf) mustBe "Hello World"
    }
    ...
    "fail on invalid ASCII" in {
      ...
      val status = result.map { _ => "success" }.recover { case _ => "failure" }
      Await.result(status, Duration.Inf) mustBe "failure"
    }
  ...
  "CharEncodingSpec.encode()" should {
    ...
    "encode US-ASCII" in {
      ...
      Await.result(result, Duration.Inf) mustBe input.mkString.getBytes("US-ASCII")
    }
    ...
    "encode UTF-8" in {
      ...
      Await.result(result, Duration.Inf) mustBe input.mkString.getBytes("UTF-8")
    }
    ...
    "encode UTF-16" in {
      ...
      Await.result(result, Duration.Inf) mustBe input.mkString.getBytes("UTF-16BE")
    }
    ...
    "encode UTF-32" in {
      ...
      Await.result(result, Duration.Inf) mustBe input.mkString.getBytes("UTF-32")
    }
    ...
    "fail on invalid Unicode with UTF-16" in {
      val input = Seq(
        "\ud83c"
      )
