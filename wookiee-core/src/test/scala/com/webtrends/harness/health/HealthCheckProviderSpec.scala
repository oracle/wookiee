...
class HealthCheckProviderSpec extends SpecificationWithJUnit {
  ...
  val rollupTests = List(
    ...
    (
      ...
      List(
        HealthComponent("test1", ComponentState.NORMAL, "", None, List(               // This should be DEGRADED since it has a DEGRADED child, but testing to ensure full hierarchy is evaluated
  ...
  rollupTests.foreach { case (testName, components, (expectedState, expectedDetails)) =>
    testName in {
      val actual = HealthCheckProvider.rollupHealth(components)
      actual.state mustEqual expectedState
      actual.details mustEqual expectedDetails
    }
  }
