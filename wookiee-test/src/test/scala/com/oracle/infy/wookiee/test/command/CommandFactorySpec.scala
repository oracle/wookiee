...
import com.oracle.infy.wookiee.command.{ArrayBean, Bean, CommandHelper, MapBean}
...
class FactoryService extends Service {
  ...
  override def addCommands(): Unit = {
    ...
    addCommand[FactoryInput, FactoryOutput](
      "Marshal", {
        case mb: MapBean =>
          FactoryInput(mb.map("value").toString)
        case ab: ArrayBean =>
          FactoryInput(ab.array.head.toString)
      }: Bean => FactoryInput, { input: FactoryInput =>
        Future.successful(FactoryOutput(input.value + "-output"))
      }, { out: FactoryOutput =>
        out.value.getBytes
      }
  ...
  "Command Factory" should {
    "Add and run a new basic Command" in {
      val result = Await.result(
        CommandHelper
          .executeCommand[FactoryInput, FactoryOutput]("Basic", FactoryInput("basic")),
        timeout.duration
      )
    ...
    "Add and run a new Marshal Command with MapBean" in {
      val result = new String(
        Await.result(
          CommandHelper
            .executeCommand[Bean, Array[Byte]]("Marshal", MapBean(Map("value" -> "marshal"))),
          timeout.duration
        )
      )
    ...
    "Add and run a new Marshal Command with ArrayBean" in {
      val result = new String(
        Await.result(
          CommandHelper
            .executeCommand[Bean, Array[Byte]]("Marshal", ArrayBean(Array("marshal"))),
          timeout.duration
        )
      )
      result shouldBe "marshal-output"
