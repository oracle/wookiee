...
class Curator(settings: ZookeeperSettings) extends LoggingAdapter {

  private[oracle] case class DiscoveryKey(basePath: String, key: String)
  private[oracle] case class ProviderKey(basePath: String, key: String)
  ...
  // list of all discovery services by base path
  ...
  def start(listener: Option[ConnectionStateListener] = None): Unit = internalClient.synchronized {
    if (internalClient.isEmpty) {
      createClient
    }
  ...
  def stop(): Unit = internalClient.synchronized {
